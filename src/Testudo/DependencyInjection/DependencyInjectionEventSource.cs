...
internal sealed class DependencyInjectionEventSource : EventSource
{
    ...
    private readonly List<WeakReference<TestudoServiceProvider>> _providers = new();
    ...
    [NonEvent]
    public void ServiceResolved(TestudoServiceProvider provider, Type serviceType)
    {
    ...
    [NonEvent]
    public void CallSiteBuilt(TestudoServiceProvider provider, Type serviceType, ServiceCallSite callSite)
    {
        if (IsEnabled(EventLevel.Verbose, EventKeywords.All))
        {
    ...
    [NonEvent]
    public void DynamicMethodBuilt(TestudoServiceProvider provider, Type serviceType, int methodSize)
    {
        if (IsEnabled(EventLevel.Verbose, EventKeywords.All))
        {
    ...
    [NonEvent]
    public void ServiceProviderBuilt(TestudoServiceProvider provider)
    {
        lock (_providers)
        {
            _providers.Add(new WeakReference<TestudoServiceProvider>(provider));
        }
    ...
    [NonEvent]
    public void ServiceProviderDisposed(TestudoServiceProvider provider)
    {
        lock (_providers)
        {
    ...
    [NonEvent]
    private void WriteServiceProviderBuilt(TestudoServiceProvider provider)
    {
        if (IsEnabled(EventLevel.Informational, Keywords.ServiceProviderInitialized))
        {
            ...
            var chunkCount = descriptorString.Length / MaxChunkSize +
...
internal static class DependencyInjectionEventSourceExtensions
{
    ...
    public static void ExpressionTreeGenerated(this DependencyInjectionEventSource source,
        TestudoServiceProvider provider,

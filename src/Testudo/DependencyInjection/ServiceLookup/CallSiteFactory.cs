...
internal sealed class CallSiteFactory : IServiceProviderIsService, IServiceProviderIsKeyedService
{
            ...
            if (serviceType.IsGenericTypeDefinition)
            {
                ...
                if (implementationType == null || !implementationType.IsGenericTypeDefinition)
                {
                    throw new ArgumentException(
                        $"Generic service {serviceType} requires generic implementation.",
                        "descriptors");
                }
                ...
                {
                    throw new ArgumentException(
                        string.Format("Abstract implementation {0} for service type {1} cannot be instantiated.",
                ...
                if (serviceTypeGenericArguments.Length != implementationTypeGenericArguments.Length)
                {
                    throw new ArgumentException(
                        string.Format(
                            "Arity of open generic service {0} not equal arity of open generic implementation {1}.",
                            serviceType,
                            implementationType), "descriptors");
                }

                if (TestudoServiceProvider.VerifyOpenGenericServiceTrimmability)
                {
        ...
        Type[] implementationTypeGenericArguments)
    {
        ...
        for (var i = 0; i < serviceTypeGenericArguments.Length; i++)
        {
            ...
            {
                throw new ArgumentException(string.Format("Trimming annotations do not match for " +
                                                          "implementation type name {0} for service type {1}.",
                    implementationType.FullName,
                    serviceType.FullName));
            }

            var serviceHasNewConstraint =
                serviceGenericType.GenericParameterAttributes.HasFlag(GenericParameterAttributes
                    .DefaultConstructorConstraint);
            var implementationHasNewConstraint =
            ...
            if (implementationHasNewConstraint && !serviceHasNewConstraint)
            {
                throw new ArgumentException(string.Format(
                    "Trimming annotations for implementation named {0} do not match for service type {1}.",
    ...
    private static bool AreCompatible(DynamicallyAccessedMemberTypes serviceDynamicallyAccessedMembers,
        ...
        serviceDynamicallyAccessedMembers.HasFlag(implementationDynamicallyAccessedMembers);
    ...
    internal ServiceCallSite? GetCallSite(ServiceIdentifier serviceIdentifier, CallSiteChain callSiteChain) =>
        _callSiteCache.TryGetValue(new ServiceCacheKey(serviceIdentifier, DefaultSlot), out var site)
            ? site
    ...
    private ServiceCallSite? TryCreateEnumerable(ServiceIdentifier serviceIdentifier, CallSiteChain callSiteChain)
    {
        ...
        try
        {
            ...
            if (TestudoServiceProvider.VerifyAotCompatibility && itemType.IsValueType)
            {
                ...
                throw new InvalidOperationException($"AOT cannot create enumerable value type {itemType}.");
            }
            ...
            else
            {
                ...
                {
                    if (KeysMatch(Descriptors[i].ServiceKey, cacheKey.ServiceKey))
                    {
                        if (TryCreateExact(Descriptors[i], cacheKey, callSiteChain, slot) is { } callSite)
                        {
                ...
                for (var i = Descriptors.Length - 1; i >= 0; i--)
                {
                    if (KeysMatch(Descriptors[i].ServiceKey, cacheKey.ServiceKey))
                    {
                        if (TryCreateOpenGeneric(Descriptors[i], cacheKey, callSiteChain, slot, false) is { } callSite)
                        {
                            AddCallSite(callSite, i);
                        }
                    }
    ...
    private static CallSiteResultCacheLocation GetCommonCacheLocation(CallSiteResultCacheLocation locationA,
        CallSiteResultCacheLocation locationB) => (CallSiteResultCacheLocation)Math.Max((int)locationA, (int)locationB);
    ...
    [UnconditionalSuppressMessage("AotAnalysis", "IL3050:RequiresDynamicCode",
        Justification =
            "When ServiceProvider.VerifyAotCompatibility is true, which it is by default when PublishAot=true, " +
            "this method ensures the generic types being created aren't using ValueTypes.")]
    private ServiceCallSite? TryCreateOpenGeneric(ServiceDescriptor descriptor, ServiceIdentifier serviceIdentifier,
        CallSiteChain callSiteChain, int slot, bool throwOnConstraintViolation)
    {
        if (serviceIdentifier.IsConstructedGenericType &&
            serviceIdentifier.ServiceType.GetGenericTypeDefinition() == descriptor.ServiceType)
        {
            ...
            try
            {
                ...
                if (TestudoServiceProvider.VerifyAotCompatibility)
                {
        ...
        try
        {
                        ...
                        foreach (var p in parameters)
                        {
                            if (!bestConstructorParameterTypes.Contains(p.ParameterType))
                            {
                                ...
                                throw new InvalidOperationException(string.Join(
                                    Environment.NewLine,
                                    $"Ambiguous constructors for {implementationType}.",
                                    bestConstructor,
                                    constructors[i]));
                            }
                        }
            ...
            if (bestConstructor == null)
            {
                throw new InvalidOperationException(
                    $"Unable to activate type {implementationType}.");
            }
            else
            {
        ...
        for (var index = 0; index < parameters.Length; index++)
        {
            ...
            foreach (var attribute in parameters[index].GetCustomAttributes(true))
            {
                if (serviceIdentifier.ServiceKey != null && attribute is ServiceKeyAttribute)
                {
                    ...
                    {
                        throw new InvalidOperationException("Invalid service key type.");
                    }
            ...
            if (callSite == null)
            {
                if (throwIfCallSiteNotFound)
                {
                    throw new InvalidOperationException(string.Format(
                        "Cannot resolve service {0} with type {1}.",
                        parameterType,
                        implementationType));
                }
    ...
    private static void VerifyOpenGenericAotCompatibility(Type serviceType, Type[] genericTypeArguments)
    {
        foreach (var typeArg in genericTypeArguments)
        {
            ...
            {
                throw new InvalidOperationException(string.Format(
                    "AOT cannot create generic value type {0}<{1}>",
                    serviceType,
                    typeArg));
            }
        }
    ...
    private struct ServiceDescriptorCacheItem
    {
        ...
        public int GetSlot(ServiceDescriptor descriptor)
        {
            ...
            throw new InvalidOperationException("Service descriptor does not exist.");
        }
